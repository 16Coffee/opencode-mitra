...
export async function ensureRunning(redirectUri?: string): Promise<void> {
  ...
  if (isHttpsRedirect(redirectUri)) {
    log.info("https redirect — skipping local oauth callback server", { redirectUri })
    return
  }
...
export function injectCode(oauthState: string, code: string): boolean {
  ...
  pending.resolve(code)
  log.info("oauth code injected from host", { oauthState })
  return true
}
...
export function injectError(oauthState: string, errorMessage: string): boolean {
  ...
  log.info("oauth error injected from host", { oauthState, errorMessage })
  return true
}

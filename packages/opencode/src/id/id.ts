...
/** Legacy time component was `ms mod 2^36` — one era spans 2^36 ms. */
const LEGACY_ERA_MS = 2 ** 36
...
/**
 * Whether an ascending ID was minted before `cutoffMs` (epoch ms). Handles
 * both formats on their own scale: wide ids compare true milliseconds; legacy
 * ids compare within their era window (`cutoffMs mod 2^36`), which stays
 * correct for the era they were minted in and degrades to "keep" (never
 * "delete early") once the legacy era has lapsed.
 */
export function olderThan(id: string, cutoffMs: number): boolean {
  const body = idBody(id)
  if (body.startsWith(WIDE_SENTINEL)) return timestamp(id) < cutoffMs
  return timestamp(id) < cutoffMs % LEGACY_ERA_MS
}

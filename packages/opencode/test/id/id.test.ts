...
describe("wide id encoding", () => {
  ...
  test("olderThan compares each format on its own scale", () => {
    const cutoffMs = Date.UTC(2026, 7, 10)
    const dayMs = 24 * 3600 * 1000

    expect(Identifier.olderThan(Identifier.create("tool", "ascending", cutoffMs - dayMs), cutoffMs)).toBe(true)
    expect(Identifier.olderThan(Identifier.create("tool", "ascending", cutoffMs + dayMs), cutoffMs)).toBe(false)
    expect(Identifier.olderThan(legacyCreate("tool", "ascending", cutoffMs - dayMs), cutoffMs)).toBe(true)
    expect(Identifier.olderThan(legacyCreate("tool", "ascending", cutoffMs + dayMs), cutoffMs)).toBe(false)
  })
